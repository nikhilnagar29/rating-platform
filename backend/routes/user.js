...
// GET /api/user/stores/:storeId/detail
// Fetch comprehensive details for a store view: store info, user's rating, all ratings
userRouter.get('/stores/:storeId/detail', authenticate, async (req, res) => {
  const userId = req.user.id; // Get authenticated user's ID
  ...
  try {
    ...
    // --- 2. Fetch User's Rating for this Store ---
    const userRatingResult = await db.query(
      `SELECT
         ...
         r.updated_at
       FROM ratings r
       WHERE r.user_id = $1 AND r.store_id = $2 AND r.status = 'active'`,
      [userId, storeId]
    );
    const userRating = userRatingResult.rows.length > 0 ? userRatingResult.rows[0] : null;

    // --- 3. Fetch All Ratings for this Store (excluding the current user's) ---
    // Note: Adjust limit/page if pagination is needed for ratings list
    const allRatingsResult = await db.query(
      `SELECT
       ...
       WHERE r.store_id = $1 AND r.user_id != $2 AND r.status = 'active'
       ORDER BY r.created_at DESC
       LIMIT 100`, // Adjust limit as needed
      [storeId, userId]
    );
    const allRatings = allRatingsResult.rows;

    // --- 4. Respond with all data ---
    res.json({
      store: store,
      userRating: userRating, // Will be null if user hasn't rated
      allRatings: allRatings
    });

  } catch (err) {
    console.error(`Error fetching detailed store view data (Store: ${storeId}, User: ${userId}):`, err);
    res.status(500).json({ message: 'Server error while fetching store details.' });
  }
});
